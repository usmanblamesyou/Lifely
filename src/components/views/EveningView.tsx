...
function habitMatchesEvening(habit: Habit): boolean {
  if (!habit.time_of_day) return true;
  if (Array.isArray(habit.time_of_day)) {
    return habit.time_of_day.length === 0 || habit.time_of_day.includes('evening');
  }
  if (typeof habit.time_of_day === 'string') {
    return habit.time_of_day === '' || (habit.time_of_day as string).includes('evening');
  }
  return true;
}
...
export default function EveningView() {
  ...
  const fetchForDate = async () => {
    ...
    try {
      ...
      console.error('Failed to fetch evening view data:', err);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="today-view">
      <div className="today-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <h2 className="today-title text-title">Evening Overview</h2>
        </div>
      </div>
      ...
      <div className="today-content">
        {isLoading ? (
          ...
          <div className="empty-state-improved">
            ...
            <div className="empty-state-headline">Nothing scheduled for evening</div>
            <div className="empty-state-subtitle">Add an evening habit or task for this day.</div>
      ...
      <NewHabitForm
        ...
        onSuccess={fetchForDate} initialStartDate={dateString} initialTimeOfDay={['evening']}
        editHabit={editHabit}
      />
      <NewTaskForm
